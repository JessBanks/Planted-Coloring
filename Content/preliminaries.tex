...
\begin{theorem}[Local Statistics]
    Let $C_{H,S,d}$ be the number of occurrences of $(H,S)$ in a rooted $d$-regular tree in which some vertex in $S$ is mapped to the root. Then, if $(H,S,\tau)$ is a partially labeled graph with $O(1)$ edges, $\ell$ connected components, and at most two distinguished vertices, then in any RSBM $\Planted_{d,k,M,\pi}$,
    \begin{enumerate}
